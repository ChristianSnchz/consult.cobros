...
describe('useDatepicker', () => {
  ...
  test.each([
    ...
    ({ blockedDate, minBookingDate, maxBookingDate, selectedDate, expectedStartDate, expectedEndDate }) => {
      const onDatesChange = jest.fn();
      advanceTo(new Date(2019, 2, 27, 0, 0, 0));
      const { result } = renderHook(() =>
        useDatepicker({
...
describe('isDateBlocked', () => {
  ...
  test('should not be blocked', () => {
    ...
    expect(isDateBlocked({ startDate: null, endDate: null, date: new Date(2019, 2, 10, 0, 0, 0) })).toBe(false);
    expect(
      isDateBlocked({
        startDate: null,
        endDate: null,
...
describe('getInputValue', () => {
  test('should return formatted value', () => {
    ...
    expect(getInputValue(date, (d: Date) => format(d, 'yyyy'), 'default value')).toBe('2019');
  });
...
describe('isDateHovered', () => {
  test.each([
    ...
    // eslint-disable-next-line no-shadow
    ({ startDate, endDate, hoveredDate, isDateBlocked, date, expected, exactMinBookingDays, minBookingDays }) => {
      expect(
        isDateHovered({
          startDate,
          endDate,
